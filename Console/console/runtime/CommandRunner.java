...
import console.commands.exceptions.InvalidVariableException;
import console.commands.exceptions.InvalidSyntaxException;
import console.display.Console;
import console.display.EFrame;
import console.display.EText;
...
public final class CommandRunner extends Thread implements Runnable
{
     ...
     * @throws InvalidVariableException When an assumption call leads to an assumption which isn't registered
     * @see getArguments
     */
    	...
    	if(args.length > 0)
	    	for(int i = 0; i < args.length; i++)
	    		if(args[i].length() > 0)
		        	...
		        	{
		        		if(args[i].length() < 2)
		        			throw new InvalidVariableException("Variable name is not given");
		        		if(var.isVariable(args[i].substring(1)))
		        			args[i] = console.getVarAsString(args[i].substring(1));
		        		else
		        			throw new InvalidVariableException(args[i].substring(1) + " is not a valid Variable");
		        	}
		    		else
		    			args[i].replaceAll("_", " ");
    	return args;
