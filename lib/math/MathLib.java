...
public class MathLib 
{
	...
	public static double calculate(String input) throws InvalidArgumentException
	{
		...
		for(int i = 0; i < toChange.length(); i++)
		{
			...
			else if(ch == '(' || ch == '[')
			{
				...
				if(lastIndex == -1)
					throw new InvalidArgumentException("The function is not enclosed");	
		...
		for(int i = 0; i < toChange.length(); i++)
		{
			...
			if(ch == '*' || ch == '/')
			{
				if(i+1 == toChange.length())
					throw new InvalidArgumentException("An operation is missing a parameter");
	...
	private static double convert(String str) throws InvalidArgumentException
	{
		try
		{
			boolean dot = false;
			for(int i = 0; i < str.length(); i++)
			{
				...
				{
					if(dot)
						throw new InvalidArgumentException("There are too many dots in a given argument");
					dot = true;
				}
			}
		...
		{
			throw new InvalidArgumentException("Issue while parsing.");
		}
	}

	private static double function(String line) throws InvalidArgumentException
	{
		...
		else if(functionName.equalsIgnoreCase("fact"))
		{
			...
			if(isInt(ans))
				...
				throw new InvalidArgumentException("The fact(x) function requires x to be an integer");
		}
		...
		else if(functionName.equalsIgnoreCase("root"))
		{
			...
			throw new InvalidArgumentException("The given function isn't valid");
	}

	private static boolean isInt(double conv) throws InvalidArgumentException
	{
