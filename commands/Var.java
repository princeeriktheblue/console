...
import commands.exceptions.CommandLoadingException;
...
public class Var extends Command implements Storable
{
	...
	public synchronized void execute(String[] args) throws InvalidCommandException 
	{
		...
		switch(code)
		{
			...
			case 2:
			{
				if(args.length == 4 + ind)
				{
					...
					name = args[2 + ind];
					TableVariable var = new TableVariable(args[2 + ind]);
					try 
					{
		...
		if(makevar)
		{
			...
			try
			{
				Variable v = Variable.makeVariable(name, vData);
				if(save)
					tosave.add(v);
				else
					vars.add(v);
			...
			{
				throw new InvalidArgumentException("The data given doesn't correspond or save as a variable");
			}
	...
	{
		File file = ESystem.getLocalFile("variables.DATA");
		if(file == null)
			console.output("No variables file was found");
		else
		{
	...
	public void saveData() 
	{
    	...
    	if(file == null)
    	{
    		file = new File(System.getProperty("user.dir") + "variables.DATA");
    		try 
    		{
	...
	private synchronized void addVariable(String name, String data) throws CommandErrorException
    {
		try 
		{
			Variable toAdd = Variable.makeVariable(name, data);
			if(toAdd == null)
		...
		{
    		throw new CommandLoadingException("Error: Invalid variable data");
		}
    }
	 ...
	 */
	private EList<Variable> vars;
	/**
	 * Saving variables to be loaded and saved
	 */
	private EList<Variable> tosave;
}